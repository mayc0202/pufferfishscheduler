...
import com.pufferfishscheduler.plugin.common.model.DatabaseField;
import com.pufferfishscheduler.plugin.common.model.DatabaseTableField;
...
public final class SqlConditionBuilder {
    ...
    public static String generateQuerySql(String dbType, Integer pageNo, Integer pageSize,
                                          ...
                                          String tableName, String searchKeyword) throws KettleException {
        switch (dbType) {
            ...
            default:
                throw new KettleException("暂不支持此数据源类型！");
        }
    }
    ...
    // -------------------------------------------------------------------------
    public static String buildSelectSql(JSONObject conditionJson, List<DatabaseTableField> fieldCache) throws KettleException {
        if (fieldCache == null || fieldCache.size() < 2) {
            logger.error("字典表：字段缓存至少需要两列");
            throw new KettleException("[值映射]字典表：字段缓存至少需要两列");
        }
    ...
    // -------------------------------------------------------------------------
    public static String buildWhereClause(JSONObject condition, String dbType) throws KettleException {
    ...
    private static String buildGroupOrCondition(JSONObject group, String dbType) throws KettleException {
     ...
     * 校验条件 JSON 格式
     */
    private static void validateConditionJson(JSONObject condition) throws KettleException {
        if (!condition.containsKey("condition") || !condition.containsKey("conditionGroups")) {
            logger.error("字典表校验：条件格式错误，请检查条件配置！");
            throw new KettleException("[值映射]字典表校验：条件格式错误，请检查条件配置！");
        }
    }
     ...
     * 构建扁平条件
     */
    private static void appendFlatConditions(StringBuilder where, JSONArray conditions, String joinOp, String dbType) throws KettleException {
     ...
     * 构建分组条件
     */
    private static void appendGroupedConditions(StringBuilder where, JSONArray conditions, String joinOp, String dbType) throws KettleException {
        if (conditions == null || conditions.isEmpty()) return;
    ...
    // 数据类型转换 & 过滤表达式构建
    // -------------------------------------------------------------------------
    public static Object convertValueByDataType(String dataType, Object filterValue) throws KettleException {
        if (StringUtils.isBlank(dataType)) {
            logger.error("数据库字典表：数据类型不能为空");
            throw new KettleException("[值映射]数据库字典表：数据类型不能为空");
        }
            ...
            switch (dt) {
                ...
                case "char":
                    if (StringUtils.isBlank(strVal)) {
                        logger.error("数据库字典表：char 类型过滤值为空");
                        throw new KettleException("[值映射]数据库字典表：char 类型过滤值为空");
                    }
                ...
                default:
                    logger.error("数据库字典表：不支持的数据类型：" + dataType);
                    throw new KettleException("[值映射]数据库字典表：不支持的数据类型：" + dataType);
            }
        } catch (NumberFormatException e) {
            logger.error("数据库字典表：数值格式错误：" + e.getMessage());
            throw new KettleException("[值映射]数值格式错误：" + e.getMessage());
        } catch (ParseException e) {
            logger.error("数据库字典表：日期解析失败：" + e.getMessage());
            throw new KettleException("[值映射]日期解析失败：" + e.getMessage());
        }
    }

    public static String buildFilterExpression(String dbType, String column, String op, Object value) throws KettleException {
        StringBuilder sb = new StringBuilder();
        ...
        switch (op) {
            ...
            default:
                logger.error("数据库字典表：不支持的操作符：" + op);
                throw new KettleException("[值映射]数据库字典表：不支持的操作符：" + op);
        }
        return sb.toString();
    }
    ...
    // SQL 语法校验
    // -------------------------------------------------------------------------
    public static void validateSqlSyntax(String sql, DatabaseConnectionInfo connInfo) throws KettleException {
        try (Connection conn = JdbcUtil.getConnection(
            ...
            logger.error("数据库字典表：SQL 语法校验失败：" + e.getMessage());
            throw new KettleException("[值映射]数据库字典表：SQL 语法校验失败：" + e.getMessage());
        }
    }

    public static void validateCustomSql(String sql) throws KettleException {
        if (sql.contains("*")) {
            logger.error("数据库字典表：自定义SQL禁止使用 *，请明确指定两列字段");
            throw new KettleException("[值映射]数据库字典表：自定义SQL禁止使用 *，请明确指定两列字段");
        }
        if (isInvalidColumnCount(sql)) {
            logger.error("数据库字典表：自定义SQL只能查询两列字段");
            throw new KettleException("[值映射]数据库字典表：自定义SQL只能查询两列字段");
        }
    }
    ...
    public static Object readColumnValue(ResultSet rs, ResultSetMetaData meta, int idx) throws SQLException {
        return switch (meta.getColumnType(idx)) {
            case Types.INTEGER -> rs.getInt(idx);
            case Types.BIGINT -> rs.getBigDecimal(idx);
            case Types.FLOAT -> rs.getFloat(idx);
            case Types.DOUBLE -> rs.getDouble(idx);
            case Types.VARCHAR -> rs.getString(idx);
            case Types.BOOLEAN -> rs.getBoolean(idx);
            case Types.DATE -> rs.getDate(idx);
            case Types.TIME -> rs.getTime(idx);
            case Types.TIMESTAMP -> rs.getTimestamp(idx);
            default -> rs.getObject(idx);
        };
    }
     ...
     */
    public static String buildFullTableName(String dbType, String schema, String dbName, String tableName) throws KettleException {
        // 表名不能为空
        if (StringUtils.isBlank(tableName)) {
            logger.error("数据库字典表：表名不能为空！");
            throw new KettleException("[值映射]数据库字典表：表名不能为空！");
        }
