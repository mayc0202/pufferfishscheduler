 ...
 * Null替换为指定值
 */
public class ReplaceNullToSpecifyValue implements RuleProcessor {
    ...
    public void init(JSONObject metadata) throws KettleStepException {
        ...
        if (fieldList != null) {
            for (Object param : fieldList) {
                ...
                if (fieldValue != null) {
                    if (name.equals(Constants.SPECIFY_VALUE)) {
                        // 指定值数据类型转换
                        switch (data.getString(Constants.RENAME_TYPE)) {
                            case Constants.NUMBER:
                                specifyValue = Double.parseDouble(fieldValue.toString());
                                break;
                            case Constants.INTEGER:
                                specifyValue = Integer.parseInt(fieldValue.toString());
                                break;
                            default:
                                specifyValue = fieldValue.toString();
                        }
                    }
                }
            }
