...
import com.pufferfishscheduler.master.collect.trans.engine.entity.JobEntryExecutingInfo;
...
@Slf4j
@AllArgsConstructor
public class KettleStepLogListener implements StepListener {
    ...
    private void updateLogChannel(String stepName, boolean executeResult, StepStatistics statistics) {
        if (executeResult) {
            LogChannelManager.get(logChannelId).addLog(Constants.EXECUTE_STATUS.SUCCESS,
            ...
            LogChannelManager.get(logChannelId).addLog(Constants.EXECUTE_STATUS.FAILURE,
                    String.format("步骤“%s”执行失败！", stepName));
        }

        LogChannelManager.get(logChannelId).addStepStatus(stepName,
    ...
    private void handleJobResult(TransWrapper transWrapper, JobExecutingInfo jobExecutingInfo) {
        if (getLongValue(jobExecutingInfo.getNrLinesError()) > 0 || (jobExecutingInfo.getResult() != null && !jobExecutingInfo.getResult())) {
            ...
            LogChannelManager.get(logChannelId).addLog(Constants.EXECUTE_STATUS.FAILURE, "转换执行失败！");
        }
    }
    ...
    private static class StepStatistics {
        ...
        public long getNrLinesRead() { return nrLinesRead; }
        public long getNrLinesWrite() { return nrLinesWrite; }
        public long getNrLinesInput() { return nrLinesInput; }
        public long getNrLinesUpdated() { return nrLinesUpdated; }
        public long getNrLinesOutput() { return nrLinesOutput; }
        public long getNrLinesRejected() { return nrLinesRejected; }
        public long getNrLinesError() { return nrLinesError; }
        public long getNrLinesDeleted() { return nrLinesDeleted; }
        public long getNrReslutFiles() { return nrReslutFiles; }
        public long getNrResultRow() { return nrResultRow; }
    }
}
