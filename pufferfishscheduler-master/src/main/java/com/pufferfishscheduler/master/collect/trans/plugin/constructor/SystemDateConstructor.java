 ...
 * 系统日记
 */
public class SystemDateConstructor extends AbstractStepMetaConstructor {
    ...
    public StepMeta create(String config, TransMeta transMeta, StepContext context) {
        ...
        SystemDataTypes[] types = new SystemDataTypes[fieldList.size()];

        for (int i = 0; i < fieldList.size(); i++) {
            JSONObject jo = fieldList.getJSONObject(i);
            fieldNames[i] = jo.getString("name");
            types[i] = SystemDataTypes.getTypeFromString(jo.getString("type"));
        }

        systemDataMeta.setFieldName(fieldNames);
        systemDataMeta.setFieldType(types);

        String eiPluginId = context.getRegistryID().getPluginId(StepPluginType.class, systemDataMeta);
        StepMeta stepMeta = context.getStepMetaMap().get(context.getId());
        if (null == stepMeta) {
            stepMeta = new StepMeta(eiPluginId, name, systemDataMeta);
        } else {
            stepMeta.setStepID(eiPluginId);
            stepMeta.setName(name);
            stepMeta.setStepMetaInterface(systemDataMeta);
        }
