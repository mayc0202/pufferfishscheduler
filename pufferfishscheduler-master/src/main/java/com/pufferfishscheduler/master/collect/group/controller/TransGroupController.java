package com.pufferfishscheduler.master.collect.group.controller;
...
public class TransGroupController {
     ...
     * @return 转换分组树形结构（仅分组）
     */
    @Operation(summary = "查询转换分组列表（仅分组）")
    @GetMapping("/groupTree.do")
    public ApiResponse groupTree(@RequestParam(required = false) String name) {
        return ApiResponse.success(transGroupService.groupTree(name));
    }
