...
public class RabbitMqConsumerInputConstructor extends AbstractStepMetaConstructor {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RabbitMqConsumerInputConstructor.class);

    private static final String PLUGIN_ID = "RabbitMqConsumerInput";
    ...
    public StepMeta create(String config, TransMeta transMeta, StepContext context) {
        ...
        if (fieldList != null && !fieldList.isEmpty()) {
            for (int i = 0; i < fieldList.size(); i++) {
                ...
                if (StringUtils.isBlank(rabbitName)) {
                    rabbitName = field.getString("kafkaName");
                }
        ...
        if (StringUtils.isBlank(pluginId)) {
            pluginId = PLUGIN_ID;
            LOGGER.warn("PluginRegistry 未解析到 RabbitMQ 输入 pluginId（步骤：{}），已回退为 {}", name, PLUGIN_ID);
        }
