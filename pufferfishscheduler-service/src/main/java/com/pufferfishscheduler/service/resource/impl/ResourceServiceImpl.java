package com.pufferfishscheduler.service.resource.impl;
...
import com.pufferfishscheduler.service.database.connect.ftp.FTPManager;
...
//            normalizedPath = ftpPath + Constants.FILE_SEPARATOR + handleBuiltInDirectory(normalizedPath);
    ...
    private String buildPath() {
        ...
        SimpleDateFormat formatter = new SimpleDateFormat(Constants.DEFAULT_DATE_FORMAT_2);
        String date = formatter.format(currentTime);
        return ftpPath + File.separator + date + File.separator + uuid;
    }
    ...
    public void download(HttpServletResponse response, ResourceForm form) {
        if (StringUtils.isBlank(form.getName())) {
            throw new BusinessException("请校验资源名称是否为空!");
        }

        if (StringUtils.isBlank(form.getPath())) {
            throw new BusinessException("请校验资源当前路径是否为空!");
        }
        ...
        try {
            ...
            try {
                ...
                throw new BusinessException("文件名称转换失败！");
            }
    ...
    private void ftpDirectDownload(DbDatabase database, ResourceForm form, HttpServletResponse response) {
        ...
        try {
            ...
            // 构建远程文件路径
            String remotePath = ftpPath + Constants.FILE_SEPARATOR + handleBuiltInDirectory(form.getPath());
            String remoteFileName = form.getName();
            String fullRemotePath = buildPath(remotePath, remoteFileName);

            // 检查文件是否存在
            FTPFile[] remoteFiles = ftpClient.listFiles(fullRemotePath);
            if (ArrayUtils.isEmpty(remoteFiles)) {
                throw new BusinessException("远程文件不存在: " + fullRemotePath);
            }

            FTPFile remoteFile = remoteFiles[0];
            if (!remoteFile.isFile()) {
                throw new BusinessException("指定路径不是文件: " + fullRemotePath);
            }

            // 设置文件大小头信息（可选）
            response.setHeader(Constants.HEADER_CONFIG.CONTENT_LENGTH, String.valueOf(remoteFile.getSize()));

            // 直接下载到响应流
            boolean success = ftpClient.retrieveFile(fullRemotePath, response.getOutputStream());

            if (!success) {
                throw new BusinessException("FTP文件下载失败: " + fullRemotePath);
            }

            log.info("FTP文件直接下载成功: {}", fullRemotePath);

        } catch (Exception e) {
            log.error("FTP直接下载异常: {}", e.getMessage(), e);
            throw new BusinessException("FTP文件下载失败: " + e.getMessage());
        } finally {
            try {
                ftpManager.close();
            } catch (Exception e) {
    ...
    private void ftpsDirectDownload(DbDatabase database, ResourceForm form, HttpServletResponse response) {
        ...
        try {
            ...
            // 检查文件是否存在
            FTPFile[] remoteFiles = ftpsClient.listFiles(fullRemotePath);
            if (ArrayUtils.isEmpty(remoteFiles)) {
                throw new BusinessException("远程文件不存在: " + fullRemotePath);
            }
            ...
            // 设置文件大小头信息（可选）
            response.setHeader("Content-Length", String.valueOf(remoteFile.getSize()));

            // 直接下载到响应流
            boolean success = ftpsClient.retrieveFile(fullRemotePath, response.getOutputStream());

            if (!success) {
                throw new BusinessException("FTPS文件下载失败: " + fullRemotePath);
            }

            log.info("FTPS文件直接下载成功: {}", fullRemotePath);

        } catch (Exception e) {
            log.error("FTPS直接下载异常: {}", e.getMessage(), e);
            throw new BusinessException("FTPS文件下载失败: " + e.getMessage());
        } finally {
            try {
                ftpsManager.close();
            } catch (Exception e) {
    ...
    private String getContentType(String fileName) {
        if (StringUtils.isBlank(fileName)) {
            return Constants.CONTENT_TYPE.OCTET_STREAM;
        }

        String extension = fileName.substring(fileName.lastIndexOf(".") + 1).toLowerCase();

        switch (extension) {
            case Constants.FILE_TYPE.TXT:
                return Constants.CONTENT_TYPE.TXT;
            case Constants.FILE_TYPE.PDF:
                return Constants.CONTENT_TYPE.PDF;
            case Constants.FILE_TYPE.ZIP:
                return Constants.CONTENT_TYPE.ZIP;
            case Constants.FILE_TYPE.XLSX:
            case Constants.FILE_TYPE.XLS:
                return Constants.CONTENT_TYPE.XLS;
            case Constants.FILE_TYPE.DOCX:
            case Constants.FILE_TYPE.DOC:
                return Constants.CONTENT_TYPE.DOC;
            case Constants.FILE_TYPE.PPTX:
            case Constants.FILE_TYPE.PPT:
                return Constants.CONTENT_TYPE.PPT;
            case Constants.FILE_TYPE.JPG:
            case Constants.FILE_TYPE.JPEG:
                return Constants.CONTENT_TYPE.JPG;
            case Constants.FILE_TYPE.PNG:
                return Constants.CONTENT_TYPE.PNG;
            case Constants.FILE_TYPE.GIF:
                return Constants.CONTENT_TYPE.GIF;
            case Constants.FILE_TYPE.XML:
                return Constants.CONTENT_TYPE.XML;
            case Constants.FILE_TYPE.JSON:
                return Constants.CONTENT_TYPE.JSON;
            default:
                return Constants.CONTENT_TYPE.OCTET_STREAM;
        }
    }
     ...
     */
    private void setDownloadHeader(HttpServletResponse response,
                                   String fileName,
                                   String contentType)
            throws UnsupportedEncodingException {
        String encodedName = URLEncoder.encode(fileName, Constants.CONTROL_ENCODING)
                .replaceAll("\\+", "%20");
        response.reset();
        response.setCharacterEncoding(Constants.CONTROL_ENCODING);
        response.setContentType(contentType);
        response.setHeader(Constants.HEADER_CONFIG.CONTENT_DISPOSITION, "attachment; filename*=UTF-8''" + encodedName);
