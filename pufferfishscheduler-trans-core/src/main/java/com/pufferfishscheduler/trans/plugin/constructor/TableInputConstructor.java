...
public class TableInputConstructor extends AbstractStepMetaConstructor {
    ...
    public StepMeta create(String config, TransMeta transMeta, StepContext context) {
        ...
        TransPluginRuntime pluginRuntime = PufferfishSchedulerApplicationContext.getBean(TransPluginRuntime.class);
        ...
        DatabaseMeta dataMeta = createDatabaseMeta(dataSourceId, flowId, pluginRuntime, aesUtil, data, sql, increment,
                tableInput);
     ...
     * @param flowId          流程ID
     * @param pluginRuntime   宿主运行时（数据源、增量 SQL 等）
     * @param aesUtil         加密工具
     * @param data            配置数据
     * @param sql             SQL语句
    ...
    private DatabaseMeta createDatabaseMeta(String dataSourceId, Integer flowId,
            TransPluginRuntime pluginRuntime, AESUtil aesUtil,
            JSONObject data, String sql, boolean increment,
            TableInputMeta tableInput) {
        DatabaseMeta dataMeta = new DatabaseMeta();

        if (StringUtils.isNotBlank(dataSourceId)) {
            DbDatabase database = pluginRuntime.getDatabaseById(Integer.valueOf(dataSourceId));
            if (database == null) {
            ...
            // 处理增量查询
            if (increment) {
                sql = handleIncrementQuery(pluginRuntime, database, data, sql, tableInput);
            }
     ...
     * @return 处理后的SQL语句
     */
    private String handleIncrementQuery(TransPluginRuntime pluginRuntime, DbDatabase database, JSONObject data,
            String sql, TableInputMeta tableInput) {
        ...
        sql = pluginRuntime.spellIncrementSql(database.getType(), sql, incrementType, incrementField);
